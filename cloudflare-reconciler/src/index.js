...
async function reconcileDistributionStats(env) {
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/' || url.pathname === '/health') {
      return json(await healthSnapshot(env));
    }
  ...
  async scheduled(_controller, env, context) {
    context.waitUntil(
      reconcileDistributionStats(env).catch(async (error) => {
        console.error('Scheduled reconciliation failed', error);
        await setState(env, 'last_reconciliation_error_at', Math.floor(Date.now() / 1000));
      }),
    );
  },
};
